...
- **Page-level `#!bg=`/`#!fg=` headers stay 3-hex-only, by deliberate choice.** Guide.py doesn't explicitly restrict the header value's length the way it restricts the inline `` `Fxxx ``/`` `Bxxx `` tokens, and NomadNet's colour-rendering helpers would technically accept a 6-hex value here too — but with no marker distinguishing "3-hex" from "6-hex" (the inline tags would need something like the `T` prefix for that), allowing both means a value's meaning silently depends on its length. One fixed width, applied consistently to every colour value in this converter, is safer than chasing every length NomadNet's code happens to tolerate.
